...
func (r *SQLiteRepository) FinanceOverviewStats(startOfDay, endOfDay time.Time) FinanceOverviewStats {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	_ = r.db.QueryRow(`SELECT COALESCE(SUM(CASE WHEN enabled <> 0 THEN 1 ELSE 0 END), 0) FROM clients`).Scan(&stats.ActiveClients)
	_ = r.db.QueryRow(`
...
func (r *SQLiteRepository) FinanceEntityCounts() FinanceEntityCounts {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	_ = r.db.QueryRow(`SELECT COALESCE(SUM(CASE WHEN enabled <> 0 THEN 1 ELSE 0 END), 0) FROM clients`).Scan(&counts.ActiveClients)
	return counts
}

func (r *SQLiteRepository) FinanceTotalSpendNanoUSD() int64 {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *SQLiteRepository) financeReconcileIssueCountLocked(now time.Time) int {
...
func (r *SQLiteRepository) ListFinanceReconcileIssues(now time.Time) []FinanceReconcileIssueSummary {
	if now.IsZero() {
		now = time.Now().UTC()
	}
	r.mu.RLock()
...
func (r *SQLiteRepository) ListPaymentIncomeByDay(startedAt time.Time, days int) []PaymentIncomeDaySummary {
...
func (r *SQLiteRepository) ListFinanceTopUsersBySpend(limit int) []FinanceUserSummary {
	if limit <= 0 {
		limit = 5
	}
	if limit > 100 {
...
func (r *SQLiteRepository) ListFinanceUserSummariesPage(offset, limit int) ([]FinanceUserSummary, int) {
	if offset < 0 {
		offset = 0
	}
	if limit <= 0 {
...
func (r *SQLiteRepository) ListFinanceTopClientsBySpend(limit int) []FinanceClientSpendSummary {
	if limit <= 0 {
		limit = 5
	}
	if limit > 100 {
...
func (r *SQLiteRepository) ListTokenUsageDailySummaries(startedAt time.Time, days int, limit int) []TokenUsageDailySummary {
	if days <= 0 || startedAt.IsZero() {
		return nil
	}
...
func (r *SQLiteRepository) ListTokenUsageDailyUserSummaries(startedAt time.Time, days int, limit int) []TokenUsageDailySummary {
	if days <= 0 || startedAt.IsZero() {
		return nil
	}
	if limit <= 0 {
