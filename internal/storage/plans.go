...
func cloneBillingPlan(plan core.BillingPlan) core.BillingPlan {
	return plan
}
...
func cloneBillingPlanGroup(group core.BillingPlanGroup) core.BillingPlanGroup {
	return group
}

func cloneUserPlanEntitlement(entitlement core.UserPlanEntitlement) core.UserPlanEntitlement {
	return entitlement
}
...
func sortPlanSubscriptionSummaries(items []PlanSubscriptionSummary) []PlanSubscriptionSummary {
	slices.SortFunc(items, func(a, b PlanSubscriptionSummary) int {
		if !a.Entitlement.PurchasedAt.Equal(b.Entitlement.PurchasedAt) {
			if a.Entitlement.PurchasedAt.After(b.Entitlement.PurchasedAt) {
				return -1
			}
			return 1
		}
		return strings.Compare(a.Entitlement.ID, b.Entitlement.ID)
	})
	return items
}
...
func planSubscriptionMatchesQuery(item PlanSubscriptionSummary, query PlanSubscriptionQuery) bool {
	query = normalizePlanSubscriptionQuery(query)
	entitlement := item.Entitlement
	if query.UserID != "" &&
		!strings.EqualFold(strings.TrimSpace(entitlement.UserID), query.UserID) &&
		!strings.EqualFold(strings.TrimSpace(item.Username), query.UserID) {
		return false
	}
	if query.PlanID != "" && strings.TrimSpace(entitlement.PlanID) != query.PlanID {
		return false
	}
	if query.Status != "" && entitlement.Status != query.Status {
		return false
	}
	return true
}

func planSubscriptionUsedNanoUSD(entitlement core.UserPlanEntitlement) int64 {
	if entitlement.Status != core.UserPlanEntitlementActive || entitlement.PeriodQuotaNanoUSD <= 0 {
		return 0
	}
	...
	if remaining > entitlement.PeriodQuotaNanoUSD {
		remaining = entitlement.PeriodQuotaNanoUSD
	}
	return entitlement.PeriodQuotaNanoUSD - remaining
}

func planSubscriptionRemainingNanoUSD(entitlement core.UserPlanEntitlement) int64 {
	...
	return entitlement.CurrentQuotaNanoUSD
}
...
func billingAllocationKey(requestID, clientID, source, entitlementID string) string {
