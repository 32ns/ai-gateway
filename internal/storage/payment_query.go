...
func paymentOrderMatchesQuery(order core.PaymentOrder, query PaymentOrderQuery) bool {
	if query.UserID != "" && order.UserID != query.UserID {
		return false
	}
	if query.Provider != "" && order.Provider != query.Provider {
		return false
	}
	if query.Status != "" && order.Status != query.Status {
		return false
	}
	if query.ExcludePending && order.Status == core.PaymentOrderPending {
		return false
	}
	return true
}
