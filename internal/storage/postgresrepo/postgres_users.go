...
func (r *PostgresRepository) ListUsers() []core.User {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) GetUser(id string) (core.User, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) FindUserByUsername(username string) (core.User, error) {
...
func (r *PostgresRepository) UpsertUser(user core.User) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if exists {
		...
		if err := tx.QueryRow(`SELECT created_at_ns FROM users WHERE id = ? FOR UPDATE`, user.ID).Scan(&createdAtNS); err != nil {
			_ = tx.Rollback()
			return err
		}
...
func (r *PostgresRepository) UpdateUserMetadata(user core.User) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func upsertUserMetadataTx(tx *sqlTx, user core.User) error {
...
func userBalanceTx(tx *sqlTx, userID string) (int64, error) {
	var balance int64
...
func upsertUserBalanceTx(tx *sqlTx, userID string, balanceNanoUSD int64, updatedAt time.Time) error {
	if _, err := tx.Exec(
...
func (r *PostgresRepository) TouchUserLastUsedAt(userID string, usedAt time.Time) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if err := tx.QueryRow(`SELECT payload, last_login_at_ns FROM users WHERE id = ? FOR UPDATE`, userID).Scan(&payload, &lastLoginAtNS); err != nil {
		_ = tx.Rollback()
		if errors.Is(err, sql.ErrNoRows) {
			return ErrNotFound
		}
...
func (r *PostgresRepository) MergeUsers(source core.User, target core.User) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if err := tx.QueryRow(`SELECT balance_nano_usd FROM user_balances WHERE user_id = ? FOR UPDATE`, source.ID).Scan(&previousSourceBalance); err != nil && !errors.Is(err, sql.ErrNoRows) {
		_ = tx.Rollback()
		return err
	}
	if err := tx.QueryRow(`SELECT balance_nano_usd FROM user_balances WHERE user_id = ? FOR UPDATE`, target.ID).Scan(&previousTargetBalance); err != nil && !errors.Is(err, sql.ErrNoRows) {
		_ = tx.Rollback()
		return err
	}
	now := time.Now().UTC()
...
func (r *PostgresRepository) SetUserBalance(userID string, balanceNanoUSD int64) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) AdjustUserBalance(userID string, deltaNanoUSD int64, note string) (int64, int64, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if err := tx.QueryRow(`SELECT balance_nano_usd FROM user_balances WHERE user_id = ? FOR UPDATE`, userID).Scan(&previousBalance); err != nil {
		if !errors.Is(err, sql.ErrNoRows) {
			_ = tx.Rollback()
			return 0, 0, err
		}
...
func (r *PostgresRepository) deleteUserDataTx(tx *sqlTx, userID, email string, clientIDs []string, now time.Time) error {
...
func (r *PostgresRepository) deleteClientDataTx(tx *sqlTx, clientID string) error {
...
func (r *PostgresRepository) releaseReservedBillingForDeletedDataTx(tx *sqlTx, userID string, clientIDs []string, now time.Time) error {
	userID = strings.TrimSpace(userID)
...
func (r *PostgresRepository) UpsertUserSession(session core.UserSession) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) GetUserSession(tokenHash string) (core.UserSession, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) DeleteUserSession(tokenHash string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) DeleteUserSessionsByUser(userID string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) DeleteExpiredUserSessions(now time.Time) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) CreateEmailVerificationCode(code core.EmailVerificationCode) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) LatestEmailVerificationCode(purpose, email string) (core.EmailVerificationCode, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) CountEmailVerificationCodesSince(purpose, email string, since time.Time) int {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) UpdateEmailVerificationCode(code core.EmailVerificationCode) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) DeleteEmailVerificationCode(id string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) CreatePasswordResetToken(token core.PasswordResetToken) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) GetPasswordResetTokenByHash(tokenHash string) (core.PasswordResetToken, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) LatestPasswordResetToken(email string) (core.PasswordResetToken, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) CountPasswordResetTokensSince(email string, since time.Time) int {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) UpdatePasswordResetToken(token core.PasswordResetToken) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) DeletePasswordResetToken(id string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
