...
func (r *PostgresRepository) CreateSupportTicket(ticket core.SupportTicket, firstMessage core.SupportMessage) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) GetSupportTicket(id string) (core.SupportTicket, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) ListSupportMessages(ticketID string, limit int) []core.SupportMessage {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) AppendSupportMessage(ticketID string, message core.SupportMessage, ticket core.SupportTicket) (core.SupportTicket, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) DeleteSupportTicket(ticketID string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) MarkSupportTicketRead(ticketID string, user core.User, readAt time.Time) (core.SupportTicket, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) SupportUnreadCount(user core.User) int {
	userID := ""
	if !user.IsAdmin() {
		userID = user.ID
	}
	tickets, _ := r.ListSupportTicketsPage(SupportTicketQuery{UserID: userID, Limit: 200})
	count := 0
...
func (r *PostgresRepository) getSupportTicketLocked(id string) (core.SupportTicket, error) {
	var payload string
	err := r.db.QueryRow(`SELECT payload FROM support_tickets WHERE id = ?`, strings.TrimSpace(id)).Scan(&payload)
	if errors.Is(err, sql.ErrNoRows) {
		return core.SupportTicket{}, ErrNotFound
