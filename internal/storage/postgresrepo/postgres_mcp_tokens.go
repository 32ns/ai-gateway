...
func (r *PostgresRepository) UpsertMCPToken(token core.MCPToken) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) GetMCPToken(id string) (core.MCPToken, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.getMCPTokenByColumnLocked("id", strings.TrimSpace(id))
}

func (r *PostgresRepository) GetMCPTokenByHash(tokenHash string) (core.MCPToken, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.getMCPTokenByColumnLocked("token_hash", strings.TrimSpace(tokenHash))
}

func (r *PostgresRepository) ListMCPTokens(ownerUserID string) []core.MCPToken {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) DeleteMCPToken(id string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) getMCPTokenByColumnLocked(column, value string) (core.MCPToken, error) {
	if value == "" {
