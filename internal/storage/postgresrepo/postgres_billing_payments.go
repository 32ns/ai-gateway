...
func (r *PostgresRepository) GetClientSpend(clientID string) (core.ClientSpend, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) GetClientActualSpend(clientID string) (core.ClientSpend, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) ReserveBilling(input core.BillingReservationInput) (core.BillingReservation, error) {
	...
	var balance int64
	err = tx.QueryRow(`SELECT balance_nano_usd FROM user_balances WHERE user_id = ? FOR UPDATE`, input.UserID).Scan(&balance)
	if errors.Is(err, sql.ErrNoRows) {
	...
	err = tx.QueryRow(`SELECT spend_used_nano_usd, updated_at_ns FROM client_spend WHERE client_id = ? FOR UPDATE`, input.ClientID).Scan(&spendUsedNanoUSD, &spendUpdatedAtNS)
	if err != nil {
		if !errors.Is(err, sql.ErrNoRows) {
			return core.BillingReservation{}, err
		}
...
func (r *PostgresRepository) SettleBilling(input core.BillingSettlementInput) (core.BillingSettlement, error) {
	input.RequestID = strings.TrimSpace(input.RequestID)
	input.ClientID = strings.TrimSpace(input.ClientID)
	...
	var balance int64
	if err := tx.QueryRow(`SELECT balance_nano_usd FROM user_balances WHERE user_id = ? FOR UPDATE`, request.UserID).Scan(&balance); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return core.BillingSettlement{}, ErrNotFound
		}
		return core.BillingSettlement{}, err
...
func (r *PostgresRepository) ReleaseBilling(input core.BillingReleaseInput) error {
	input.RequestID = strings.TrimSpace(input.RequestID)
	input.ClientID = strings.TrimSpace(input.ClientID)
	tx, err := r.db.Begin()
	if err != nil {
...
func (r *PostgresRepository) ListBillingLedger(userID string, limit int) []core.BillingLedgerEntry {
	now := time.Now().UTC()
	r.mu.Lock()
...
func (r *PostgresRepository) listBillingUsageSpendByClient(userID string) []BillingUsageSpendSummary {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) CreatePaymentOrder(order core.PaymentOrder) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) GetPaymentOrder(id string) (core.PaymentOrder, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) GetPaymentOrderByOutTradeNo(outTradeNo string) (core.PaymentOrder, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) UpdatePaymentOrderStatus(id string, status core.PaymentOrderStatus, providerTradeNo string, paidAt *time.Time) (core.PaymentOrder, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	defer tx.Rollback()
	order, ok, err := getPaymentOrderByIDTx(tx, id)
	if err != nil {
		return core.PaymentOrder{}, err
	}
	if !ok {
	...
	if _, err := tx.Exec(`UPDATE payment_orders SET status = ?, amount_nano_usd = ?, paid_at_ns = ?, payload = ?, updated_at_ns = ? WHERE id = ?`, string(order.Status), order.AmountNanoUSD, paymentOrderPaidAtNS(order), string(payload), sqliteTimeNS(order.UpdatedAt), order.ID); err != nil {
		return core.PaymentOrder{}, err
	}
	if err := tx.Commit(); err != nil {
		return core.PaymentOrder{}, err
...
func (r *PostgresRepository) UpdatePaymentOrderProviderState(id string, update core.PaymentOrderProviderUpdate) (core.PaymentOrder, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) DeletePendingPaymentOrder(id string) (core.PaymentOrder, bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) CompletePaymentOrder(outTradeNo, providerTradeNo string, paidAmountNanoUSD int64, paidAt time.Time) (core.PaymentOrder, bool, error) {
	return r.CompletePaymentOrderWithCredits(outTradeNo, providerTradeNo, paidAmountNanoUSD, paidAt, nil)
}

func (r *PostgresRepository) CompletePaymentOrderWithCredits(outTradeNo, providerTradeNo string, paidAmountNanoUSD int64, paidAt time.Time, credits []core.PaymentOrderBalanceCredit) (core.PaymentOrder, bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	var balance int64
	if err := tx.QueryRow(`SELECT balance_nano_usd FROM user_balances WHERE user_id = ? FOR UPDATE`, order.UserID).Scan(&balance); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return core.PaymentOrder{}, false, ErrNotFound
		}
		return core.PaymentOrder{}, false, err
	...
	for index, credit := range normalizedCredits {
		...
		} else if err := tx.QueryRow(`SELECT balance_nano_usd FROM user_balances WHERE user_id = ? FOR UPDATE`, credit.UserID).Scan(&creditBalance); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return core.PaymentOrder{}, false, ErrNotFound
			}
			return core.PaymentOrder{}, false, err
...
func (r *PostgresRepository) CreatePaymentRefund(refund core.PaymentRefund) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	var balance int64
	if err := tx.QueryRow(`SELECT balance_nano_usd FROM user_balances WHERE user_id = ? FOR UPDATE`, refund.UserID).Scan(&balance); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return ErrNotFound
		}
		return err
...
func (r *PostgresRepository) CompletePaymentRefund(id, providerRefundNo, rawResponse string) (core.PaymentRefund, bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) FailPaymentRefund(id, rawResponse string) (core.PaymentRefund, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) ListPaymentRefunds(orderID string) []core.PaymentRefund {
	r.mu.RLock()
	defer r.mu.RUnlock()
