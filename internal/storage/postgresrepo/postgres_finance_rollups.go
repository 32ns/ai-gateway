...
func upsertFinanceUserRollupIdentityTx(tx *sqlTx, user core.User) error {
	_, err := tx.Exec(`
...
func setFinanceUserRollupBalanceTx(tx *sqlTx, userID string, balanceNanoUSD int64) error {
	_, err := tx.Exec(`
...
func markFinanceUserRollupDeletedTx(tx *sqlTx, userID string) error {
	_, err := tx.Exec(`UPDATE finance_user_rollups SET username = '' WHERE user_id = ?`, strings.TrimSpace(userID))
	return err
}

func addFinanceUserRechargeRollupTx(tx *sqlTx, userID string, amountNanoUSD int64, when time.Time) error {
	if amountNanoUSD == 0 {
		return nil
	}
	_, err := tx.Exec(`
		...
		ON CONFLICT(user_id) DO UPDATE SET
			...
			last_payment_at_ns = GREATEST(finance_user_rollups.last_payment_at_ns, excluded.last_payment_at_ns)
	`, strings.TrimSpace(userID), amountNanoUSD, sqliteTimeNS(when))
	return err
}

func addFinanceUserRefundRollupTx(tx *sqlTx, userID string, amountNanoUSD int64) error {
	if amountNanoUSD == 0 {
		return nil
	}
	_, err := tx.Exec(`
...
func addFinanceUserRewardRollupTx(tx *sqlTx, userID string, amountNanoUSD int64) error {
	if amountNanoUSD == 0 {
		return nil
	}
	_, err := tx.Exec(`
...
func addFinanceUserSpendRollupTx(tx *sqlTx, userID string, deltaNanoUSD int64, when time.Time) error {
	if deltaNanoUSD == 0 {
		return nil
	}
	_, err := tx.Exec(`
		...
		ON CONFLICT(user_id) DO UPDATE SET
			...
			END,
			last_spend_at_ns = GREATEST(finance_user_rollups.last_spend_at_ns, excluded.last_spend_at_ns)
...
func addFinanceUserUsageRollupTx(tx *sqlTx, userID string, deltaNanoUSD int64, planDeltaNanoUSD int64, when time.Time) error {
...
func upsertFinanceClientRollupMetadataTx(tx *sqlTx, client core.APIClient) error {
	_, err := tx.Exec(`
...
func addFinanceClientSpendRollupTx(tx *sqlTx, clientID, clientName, ownerUserID string, spendLimitNanoUSD, deltaNanoUSD int64) error {
...
func addFinanceClientUsageRollupTx(tx *sqlTx, clientID, clientName, ownerUserID string, spendLimitNanoUSD, deltaNanoUSD, planDeltaNanoUSD int64) error {
...
func transferFinanceClientOwnerSpendRollupTx(tx *sqlTx, clientID, previousOwnerID, nextOwnerID string, when time.Time) error {
...
func addFinanceModelRollupTx(tx *sqlTx, model string, requestDelta int, promptDelta, completionDelta, spendDelta int64) error {
...
func updateFinanceModelRollupForBillingChangeTx(tx *sqlTx, before, after core.BillingReservation) error {
...
func addFinanceTokenDailyRollupTx(tx *sqlTx, date, userID, username string, requestDelta int, promptDelta, cachedDelta, cacheCreationDelta, completionDelta, imageOutputDelta, totalDelta int64) error {
...
func updateFinanceTokenDailyRollupForBillingChangeTx(tx *sqlTx, before, after core.BillingReservation) error {
...
func applyFinanceTokenDailyRollupRequestTx(tx *sqlTx, request core.BillingReservation, sign int) error {
...
func (r *PostgresRepository) rebuildFinanceTokenDailyRollups() error {
	tx, err := r.db.Begin()
	if err != nil {
		return err
	}
