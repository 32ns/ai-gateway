...
func (r *PostgresRepository) ListBillingPlans() []core.BillingPlan {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) GetBillingPlan(id string) (core.BillingPlan, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return getBillingPlanTx(r.db, strings.TrimSpace(id))
}

func (r *PostgresRepository) UpsertBillingPlan(plan core.BillingPlan) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	`, plan.ID, plan.Name, plan.Description, plan.Group, boolParam(plan.Enabled), plan.PriceNanoUSD, plan.PeriodQuotaNanoUSD, plan.PeriodDurationSec, plan.PeriodCount, plan.SortOrder, sqliteTimeNS(plan.CreatedAt), sqliteTimeNS(plan.UpdatedAt))
	return err
}

func (r *PostgresRepository) DeleteBillingPlan(id string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.deleteByID("billing_plans", strings.TrimSpace(id))
}

func (r *PostgresRepository) ListBillingPlanGroups() []core.BillingPlanGroup {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *PostgresRepository) GetBillingPlanGroup(id string) (core.BillingPlanGroup, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	row := r.db.QueryRow(`
		SELECT id, name, sale_disabled, quota_price_ratio, sort_order, created_at_ns, updated_at_ns
	...
	if errors.Is(err, sql.ErrNoRows) {
		return core.BillingPlanGroup{}, storageerr.ErrNotFound
	}
	return group, err
}

func (r *PostgresRepository) UpsertBillingPlanGroup(group core.BillingPlanGroup) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	_, err := r.db.Exec(`
		...
		ON CONFLICT(id) DO UPDATE SET
			...
			sort_order = excluded.sort_order,
			updated_at_ns = excluded.updated_at_ns
	`, group.ID, group.Name, boolParam(group.SaleDisabled), group.QuotaPriceRatio, group.SortOrder, sqliteTimeNS(group.CreatedAt), sqliteTimeNS(group.UpdatedAt))
	return err
}

func (r *PostgresRepository) DeleteBillingPlanGroup(id string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.deleteByID("billing_plan_groups", strings.TrimSpace(id))
}

func (r *PostgresRepository) PurchaseBillingPlan(input core.BillingPlanPurchaseInput) (core.BillingPlanPurchase, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	plan = normalizeBillingPlan(plan)
	if !plan.Enabled {
		return core.BillingPlanPurchase{}, storageerr.ErrNotFound
	}
	...
	var balance int64
	err = tx.QueryRow(`SELECT balance_nano_usd FROM user_balances WHERE user_id = ? FOR UPDATE`, userID).Scan(&balance)
	if errors.Is(err, sql.ErrNoRows) {
		return core.BillingPlanPurchase{}, storageerr.ErrNotFound
	}
	if err != nil {
	...
	if balance < plan.PriceNanoUSD {
		return core.BillingPlanPurchase{}, storageerr.ErrInsufficientBalance
	}
...
func uniqueBillingLedgerIDTx(tx *sqlTx, kind, requestID, clientID string, ts time.Time) (string, error) {
	for attempt := 0; attempt < 1000; attempt++ {
...
func uniquePlanQuotaLedgerIDTx(tx *sqlTx, kind, entitlementID, requestID, clientID string, ts time.Time) (string, error) {
	for attempt := 0; attempt < 1000; attempt++ {
...
func (r *PostgresRepository) GrantBillingPlan(input core.BillingPlanGrantInput) (core.BillingPlanPurchase, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) ListUserPlanEntitlements(userID string) []core.UserPlanEntitlement {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) GetActiveUserPlanEntitlement(userID string) (core.UserPlanEntitlement, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if !ok {
		return core.UserPlanEntitlement{}, storageerr.ErrNotFound
	}
	if err := tx.Commit(); err != nil {
		return core.UserPlanEntitlement{}, err
	}
...
func (r *PostgresRepository) MoveUserPlanEntitlementPriority(userID, entitlementID, direction string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PostgresRepository) ListPlanSubscriptionPlanSummaries(query PlanSubscriptionQuery, limit int) []PlanSubscriptionPlanSummary {
	...
	for rows.Next() {
		...
		var purchaseCount, activeCount int64
		if err := rows.Scan(&summary.PlanID, &summary.PlanName, &summary.PlanGroup, &purchaseCount, &activeCount, &summary.RevenueNanoUSD, &summary.ActiveRemainingNanoUSD); err != nil {
			continue
		}
		if value, err := int64ToIntField("purchase_count", purchaseCount); err == nil {
			summary.PurchaseCount = value
		}
		if value, err := int64ToIntField("active_count", activeCount); err == nil {
			summary.ActiveCount = value
		}
...
func (r *PostgresRepository) planQuotaUsageRows(query PlanQuotaUsageQuery) []PlanQuotaUsageDaySummary {
	query = normalizePlanQuotaUsageQuery(query)
	_ = r.ListUserPlanEntitlements("")
...
func getBillingPlanTx(queryer interface {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return core.BillingPlan{}, storageerr.ErrNotFound
	}
	return plan, err
}
...
func scanBillingPlan(scanner billingPlanScanner) (core.BillingPlan, error) {
	...
	var periodCount, sortOrder int64
	if err := scanner.Scan(&plan.ID, &plan.Name, &plan.Description, &plan.Group, &enabled, &plan.PriceNanoUSD, &plan.PeriodQuotaNanoUSD, &plan.PeriodDurationSec, &periodCount, &sortOrder, &createdAtNS, &updatedAtNS); err != nil {
		return core.BillingPlan{}, err
	}
	var err error
	if plan.PeriodCount, err = int64ToIntField("period_count", periodCount); err != nil {
		return core.BillingPlan{}, err
	}
	if plan.SortOrder, err = int64ToIntField("sort_order", sortOrder); err != nil {
		return core.BillingPlan{}, err
	}
...
func scanBillingPlanGroup(scanner billingPlanScanner) (core.BillingPlanGroup, error) {
	...
	var saleDisabled bool
	var sortOrder int64
	var createdAtNS, updatedAtNS int64
	if err := scanner.Scan(&group.ID, &group.Name, &saleDisabled, &group.QuotaPriceRatio, &sortOrder, &createdAtNS, &updatedAtNS); err != nil {
		return core.BillingPlanGroup{}, err
	}
	group.SaleDisabled = saleDisabled
	var err error
	if group.SortOrder, err = int64ToIntField("sort_order", sortOrder); err != nil {
		return core.BillingPlanGroup{}, err
	}
...
func activeUserPlanEntitlementTx(tx *sqlTx, userID string, now time.Time) (core.UserPlanEntitlement, bool, error) {
...
func activeUserPlanEntitlementsTx(tx *sqlTx, userID string, now time.Time) ([]core.UserPlanEntitlement, error) {
	rows, err := tx.Query(`
		SELECT id, user_id, plan_id, plan_name, status, price_nano_usd, period_quota_nano_usd, base_period_quota_nano_usd,
			period_duration_sec, total_periods, remaining_periods, current_quota_nano_usd,
			priority, current_period_started_at_ns, current_period_ends_at_ns, expires_at_ns, purchased_at_ns, updated_at_ns
...
func nextUserPlanEntitlementPriorityTx(tx *sqlTx, userID string, now time.Time) (int, error) {
	entitlements, err := activeUserPlanEntitlementsTx(tx, userID, now)
	if err != nil {
		return 0, err
	}
...
func moveUserPlanEntitlementPriorityTx(tx *sqlTx, entitlements []core.UserPlanEntitlement, entitlementID, direction string) error {
	entitlementID = strings.TrimSpace(entitlementID)
	index := -1
...
func scanUserPlanEntitlement(scanner billingPlanScanner) (core.UserPlanEntitlement, error) {
	...
	var totalPeriods, remainingPeriods, priority int64
	if err := scanner.Scan(
		&entitlement.ID,
		&entitlement.UserID,
		&entitlement.PlanID,
	...
	var err error
	if entitlement.TotalPeriods, err = int64ToIntField("total_periods", totalPeriods); err != nil {
		return core.UserPlanEntitlement{}, err
	}
	if entitlement.RemainingPeriods, err = int64ToIntField("remaining_periods", remainingPeriods); err != nil {
		return core.UserPlanEntitlement{}, err
	}
	if entitlement.Priority, err = int64ToIntField("priority", priority); err != nil {
		return core.UserPlanEntitlement{}, err
	}
...
func scanPlanSubscriptionSummary(scanner billingPlanScanner) (PlanSubscriptionSummary, error) {
	...
	var err error
	if item.Entitlement.TotalPeriods, err = int64ToIntField("total_periods", totalPeriods); err != nil {
		return PlanSubscriptionSummary{}, err
	}
	if item.Entitlement.RemainingPeriods, err = int64ToIntField("remaining_periods", remainingPeriods); err != nil {
		return PlanSubscriptionSummary{}, err
	}
	if item.Entitlement.Priority, err = int64ToIntField("priority", priority); err != nil {
		return PlanSubscriptionSummary{}, err
	}
...
func insertUserPlanEntitlementTx(tx *sqlTx, entitlement core.UserPlanEntitlement) error {
	_, err := tx.Exec(`
...
func updateUserPlanEntitlementTx(tx *sqlTx, entitlement core.UserPlanEntitlement) error {
	_, err := tx.Exec(`
...
func cancelUserPlanEntitlementTx(tx *sqlTx, userID, entitlementID string, now time.Time) (core.UserPlanEntitlement, error) {
	userID = strings.TrimSpace(userID)
	entitlementID = strings.TrimSpace(entitlementID)
...
func cancelUserPlanEntitlementsTx(tx *sqlTx, userID string, now time.Time) error {
	userID = strings.TrimSpace(userID)
	if userID == "" {
		return nil
	}
...
func mergeUserPlanEntitlementsTx(tx *sqlTx, sourceID, targetID string, now time.Time) error {
...
func insertPlanQuotaLedgerTx(tx *sqlTx, entry core.PlanQuotaLedgerEntry) error {
	_, err := tx.Exec(`
...
func getUserPlanEntitlementByIDTx(tx *sqlTx, id string) (core.UserPlanEntitlement, bool, error) {
	row := tx.QueryRow(`
		SELECT id, user_id, plan_id, plan_name, status, price_nano_usd, period_quota_nano_usd, base_period_quota_nano_usd,
			period_duration_sec, total_periods, remaining_periods, current_quota_nano_usd,
			priority, current_period_started_at_ns, current_period_ends_at_ns, expires_at_ns, purchased_at_ns, updated_at_ns
...
func insertBillingFundingAllocationTx(tx *sqlTx, allocation core.BillingFundingAllocation) error {
	_, err := tx.Exec(`
...
func upsertBillingFundingAllocationActualDeltaTx(tx *sqlTx, requestID, clientID, userID, source, entitlementID string, delta int64, now time.Time) error {
	if delta == 0 {
		return nil
	}
...
func getBillingFundingAllocationTx(tx *sqlTx, requestID, clientID, source, entitlementID string) (core.BillingFundingAllocation, bool, error) {
	row := tx.QueryRow(`
...
func listBillingFundingAllocationsTx(tx *sqlTx, requestID, clientID string) ([]core.BillingFundingAllocation, error) {
	rows, err := tx.Query(`
		SELECT id, request_id, client_id, user_id, source, entitlement_id,
			reserved_nano_usd, actual_nano_usd, created_at_ns, updated_at_ns
		FROM billing_funding_allocations
...
func int64ToIntField(field string, value int64) (int, error) {
	maxInt := int64(^uint(0) >> 1)
	minInt := -maxInt - 1
	if value < minInt || value > maxInt {
		return 0, fmt.Errorf("%s is outside int range: %d", field, value)
	}
