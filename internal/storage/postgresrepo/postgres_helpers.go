...
func (r *PostgresRepository) getPayloadByID(table, id string) (string, error) {
	var payload string
...
func (r *PostgresRepository) getPayloadByIDTx(tx *sqlTx, table, id string) (string, error) {
	var payload string
...
func (r *PostgresRepository) userBalanceLocked(userID string) (int64, error) {
	var balance int64
...
func (r *PostgresRepository) upsertPayload(table, id, payload string) error {
	_, err := r.db.Exec(
...
func (r *PostgresRepository) upsertAccountStatePayload(table, accountID, payload string) error {
	_, err := r.db.Exec(
...
func (r *PostgresRepository) accountLocked(id string) (core.Account, error) {
...
func (r *PostgresRepository) upsertAccountPartsLocked(account core.Account) error {
...
func insertAuditTermsTx(tx *sqlTx, seq int64, actorText, resourceText string) error {
	for _, term := range auditIndexTerms(actorText) {
		if _, err := tx.Exec(
			`INSERT INTO audit_terms(seq, term_type, term) VALUES(?, ?, ?) ON CONFLICT DO NOTHING`,
			seq,
			"actor",
			term,
		); err != nil {
...
func upsertUserRecordTx(tx *sqlTx, user core.User, updatedAt time.Time) error {
...
func syncUserIdentityIndexesTx(tx *sqlTx, user core.User) error {
...
func (r *PostgresRepository) mergeUserClientsTx(tx *sqlTx, sourceID, targetID string, now time.Time) error {
...
func (r *PostgresRepository) mergeUserInvitersTx(tx *sqlTx, sourceID, targetID string, now time.Time) error {
	rows, err := tx.Query(`SELECT id, payload FROM users FOR UPDATE`)
	if err != nil {
		return err
	}
	defer rows.Close()
...
func (r *PostgresRepository) mergeUserSiteMessagesTx(tx *sqlTx, sourceID, targetID string, now time.Time) error {
	return r.updateUserSiteMessageTargetsTx(tx, sourceID, targetID, now)
}

func (r *PostgresRepository) removeUserSiteMessagesTx(tx *sqlTx, userID string, now time.Time) error {
	return r.updateUserSiteMessageTargetsTx(tx, userID, "", now)
}

func (r *PostgresRepository) updateUserSiteMessageTargetsTx(tx *sqlTx, oldID, newID string, now time.Time) error {
	...
	for rows.Next() {
		...
		updates = append(updates, messageUpdate{id: id, enabled: boolParam(message.Enabled), payload: string(nextPayload)})
	}
	if err := rows.Err(); err != nil {
		return err
	}
...
func mergeUserSiteMessageReadsTx(tx *sqlTx, sourceID, targetID string) error {
	...
	for _, item := range reads {
		if _, err := tx.Exec(`INSERT INTO site_message_reads(message_id, user_id, read_at_ns) VALUES(?, ?, ?) ON CONFLICT(message_id, user_id) DO UPDATE SET read_at_ns = GREATEST(site_message_reads.read_at_ns, excluded.read_at_ns)`, item.messageID, targetID, item.readAtNS); err != nil {
			return err
		}
	}
...
func (r *PostgresRepository) getOrCreateClientSpendTx(tx *sqlTx, clientID string) (core.ClientSpend, error) {
	var spend core.ClientSpend
	var updatedAtNS int64
	err := tx.QueryRow(`SELECT client_id, spend_limit_nano_usd, spend_used_nano_usd, updated_at_ns FROM client_spend WHERE client_id = ? FOR UPDATE`, clientID).
	...
	now := time.Now().UTC()
	if _, err := tx.Exec(`INSERT INTO client_spend(client_id, spend_limit_nano_usd, spend_used_nano_usd, updated_at_ns) VALUES(?, ?, 0, ?) ON CONFLICT(client_id) DO NOTHING`, clientID, client.SpendLimitNanoUSD, now.UnixNano()); err != nil {
		return core.ClientSpend{}, err
	}
	err = tx.QueryRow(`SELECT client_id, spend_limit_nano_usd, spend_used_nano_usd, updated_at_ns FROM client_spend WHERE client_id = ? FOR UPDATE`, clientID).
		Scan(&spend.ClientID, &spend.SpendLimitNanoUSD, &spend.SpendUsedNanoUSD, &updatedAtNS)
...
func getBillingRequestTx(tx *sqlTx, requestID, clientID string) (core.BillingReservation, bool, error) {
	row := tx.QueryRow(`
...
func (r *PostgresRepository) getPaymentOrderByColumn(column, value string) (core.PaymentOrder, error) {
	if value == "" {
...
func getPaymentOrderByOutTradeNoTx(tx *sqlTx, outTradeNo string) (core.PaymentOrder, bool, error) {
	var payload string
	err := tx.QueryRow(`SELECT payload FROM payment_orders WHERE out_trade_no = ? FOR UPDATE`, outTradeNo).Scan(&payload)
	if errors.Is(err, sql.ErrNoRows) {
...
func getPaymentOrderByIDTx(tx *sqlTx, id string) (core.PaymentOrder, bool, error) {
	var payload string
	err := tx.QueryRow(`SELECT payload FROM payment_orders WHERE id = ? FOR UPDATE`, strings.TrimSpace(id)).Scan(&payload)
	if errors.Is(err, sql.ErrNoRows) {
		return core.PaymentOrder{}, false, nil
	}
	if err != nil {
...
func getPaymentRefundByIDTx(tx *sqlTx, id string) (core.PaymentRefund, bool, error) {
	var payload string
	err := tx.QueryRow(`SELECT payload FROM payment_refunds WHERE id = ? FOR UPDATE`, strings.TrimSpace(id)).Scan(&payload)
	if errors.Is(err, sql.ErrNoRows) {
...
func refundablePaymentAmountTx(tx *sqlTx, order core.PaymentOrder) (int64, error) {
...
func pendingPaymentRefundAmountTx(tx *sqlTx, userID string) (int64, error) {
...
func (r *PostgresRepository) getSiteMessageLocked(id string) (core.SiteMessage, error) {
	if id == "" {
...
func expirePaymentOrdersDB(db *sqlDB, now time.Time) error {
...
func insertBillingRequestTx(tx *sqlTx, request core.BillingReservation) error {
	request.BillingSource = core.NormalizeClientBillingSource(request.BillingSource)
...
func updateBillingRequestSettlementTx(tx *sqlTx, request core.BillingReservation) error {
	request.BillingSource = core.NormalizeClientBillingSource(request.BillingSource)
	_, err := tx.Exec(`
...
func insertBillingLedgerTx(tx *sqlTx, entry core.BillingLedgerEntry) error {
	_, err := tx.Exec(`
...
func (r *PostgresRepository) deleteByID(table, id string) error {
...
func queryExistsTx(tx *sqlTx, query string, args ...any) (bool, error) {
	var marker int
...
func (r *PostgresRepository) trimAuditTx(tx *sqlTx) error {
	if r.limit <= 0 {
		return nil
	}
	if _, err := tx.Exec(
...
func (r *PostgresRepository) maybeTrimAuditTx(tx *sqlTx, now time.Time, writes int) error {
	if writes < 1 {
		writes = 1
	}
...
func (r *PostgresRepository) maybeTrimGatewayAuditTx(tx *sqlTx, now time.Time, writes int) error {
	r.retentionMu.Lock()
...
func trimGatewayAuditTx(tx *sqlTx, now time.Time, maxAge time.Duration) error {
	if maxAge <= 0 {
		return nil
	}
...
func (r *PostgresRepository) trimBillingRequestsTx(tx *sqlTx, now time.Time) error {
	return trimBillingRequestsTx(tx, now, r.usageMaxAge)
}

func (r *PostgresRepository) maybeTrimBillingRequestsTx(tx *sqlTx, now time.Time) error {
	r.retentionMu.Lock()
	defer r.retentionMu.Unlock()
...
func (r *PostgresRepository) maybeTrimBillingRequestsLocked(now time.Time) error {
	r.retentionMu.Lock()
	defer r.retentionMu.Unlock()
...
func (r *PostgresRepository) trimBillingRequestsLocked(now time.Time) error {
	return trimBillingRequestsDB(r.db, now, r.usageMaxAge)
}

func (r *PostgresRepository) maybeTrimBillingLedgerLocked(now time.Time) error {
	r.retentionMu.Lock()
	defer r.retentionMu.Unlock()
...
func (r *PostgresRepository) trimBillingLedgerLocked(now time.Time) error {
	return trimBillingLedgerDB(r.db, now, r.ledgerMaxAge)
}

func trimBillingRequestsTx(tx *sqlTx, now time.Time, maxAge time.Duration) error {
	if maxAge == 0 {
		if _, err := tx.Exec(`
...
func trimBillingRequestsDB(db *sqlDB, now time.Time, maxAge time.Duration) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}
...
func trimBillingLedgerTx(tx *sqlTx, now time.Time, maxAge time.Duration) error {
	if maxAge <= 0 {
...
func trimBillingLedgerDB(db *sqlDB, now time.Time, maxAge time.Duration) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}
...
func (r *PostgresRepository) encodeAccountConfigPayload(config storedAccountConfig) (string, error) {
...
func (r *PostgresRepository) decodeAccountConfigPayload(payload string) (storedAccountConfig, error) {
...
func (r *PostgresRepository) encodeAccountCredentialPayload(state storedAccountCredentialState) (string, error) {
	encoded := state
	var err error
...
func (r *PostgresRepository) decodeAccountCredentialPayload(payload string) (storedAccountCredentialState, error) {
...
func (r *PostgresRepository) encodeAccountRuntimePayload(state storedAccountRuntimeState) (string, error) {
...
func (r *PostgresRepository) decodeAccountRuntimePayload(payload string) (storedAccountRuntimeState, error) {
...
func (r *PostgresRepository) decodeStoredAccountPayloads(configPayload string, credentialPayload, runtimePayload sql.NullString) (core.Account, error) {
...
func (r *PostgresRepository) encodeAccountGroupPayload(group core.AccountGroup) (string, error) {
...
func (r *PostgresRepository) decodeAccountGroupPayload(payload string) (core.AccountGroup, error) {
...
func (r *PostgresRepository) encodeClientPayload(client core.APIClient) (string, error) {
...
func (r *PostgresRepository) decodeClientPayload(payload string) (core.APIClient, error) {
...
func (r *PostgresRepository) encodeSystemSettingsPayload(settings core.SystemSettings) (string, error) {
...
func (r *PostgresRepository) decodeSystemSettingsPayload(payload string) (core.SystemSettings, error) {
...
func (r *PostgresRepository) decodeStartupSystemSettingsPayload(payload string) (core.SystemSettings, error) {
	var settings core.SystemSettings
	if err := json.Unmarshal([]byte(payload), &settings); err != nil {
		return core.SystemSettings{}, err
	}
...
func (r *PostgresRepository) validateCredentialsLocked() error {
...
func (r *PostgresRepository) encryptedCredentialSampleLocked() (string, error) {
...
func (r *PostgresRepository) validateAccountCredentialsPayloadsLocked() (string, error) {
...
func (r *PostgresRepository) validateAccountGroupCredentialsPayloadsLocked() (string, error) {
...
func (r *PostgresRepository) validateSystemSettingsCredentialsPayloadsLocked() (string, error) {
...
func (r *PostgresRepository) validateClientCredentialsPayloadsLocked() (string, error) {
...
func (r *PostgresRepository) firstEncryptedPayloadLocked(table string) (string, error) {
	var payload string
	err := r.db.QueryRow(
