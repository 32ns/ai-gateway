...
func TestImageLabJobsAreScopedToCurrentUser(t *testing.T) {
	...
	expectedDataURL := "data:image/png;base64,YWxpY2Vfc2VjcmV0"
	if !storedResult.OK || storedResult.Image != expectedDataURL || storedResult.B64JSON != "" || storedResult.RemoteURL != "" {
	...
	if rec.Code != http.StatusOK || !strings.Contains(rec.Body.String(), expectedDataURL) {
		t.Fatalf("owner jobs status=%d body=%s", rec.Code, rec.Body.String())
	}
	if strings.Contains(rec.Body.String(), "alice_secret") {
		t.Fatalf("owner jobs leaked decoded image bytes: %s", rec.Body.String())
	}

	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodGet, "/images/api/jobs/"+job.snapshot.ID+"/results/0", nil)
	aliceHandler.ServeHTTP(rec, req)
	if rec.Code != http.StatusNotFound {
		t.Fatalf("removed result file endpoint status=%d body=%s", rec.Code, rec.Body.String())
	}
	...
	for name, handler := range map[string]http.Handler{
		...
		if strings.Contains(rec.Body.String(), job.snapshot.ID) || strings.Contains(rec.Body.String(), "alice_secret") || strings.Contains(rec.Body.String(), expectedDataURL) {
		...
		handler.ServeHTTP(rec, req)
		if rec.Code != http.StatusNotFound {
			t.Fatalf("%s removed result file endpoint status=%d body=%s", name, rec.Code, rec.Body.String())
		}
		if strings.Contains(rec.Body.String(), "alice_secret") {
...
func TestImageLabCleanupRemovesExpiredJobs(t *testing.T) {
	repo := storage.NewMemoryRepository()
	control := controlplane.New(repo, providers.NewRegistry(&providers.OpenAIAdapter{}))
	...
	if !result.OK || result.Image != "data:image/png;base64,ZXhwaXJlZA==" || result.B64JSON != "" {
...
func TestImageLabGenerateCreatesBackgroundTaskUsingImageGenerationsEndpoint(t *testing.T) {
	...
	expectedDataURL := "data:image/png;base64,aW1hZ2U="
	if len(snapshot.Results) != 1 || snapshot.Results[0] == nil || !snapshot.Results[0].OK || snapshot.Results[0].Image != expectedDataURL || snapshot.Results[0].MIME != "image/png" || snapshot.Results[0].RemoteURL != "" {
		t.Fatalf("snapshot results = %#v", snapshot.Results)
	}
	if _, err := os.Stat(server.imageLabResultRootDir()); !os.IsNotExist(err) {
		t.Fatalf("image result root should not be created for new results, stat err=%v", err)
	}
	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodGet, "/images/api/jobs/"+created.ID+"/results/0", nil)
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusNotFound {
		t.Fatalf("removed result file endpoint status=%d body=%s", rec.Code, rec.Body.String())
	}
